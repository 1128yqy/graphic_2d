...
void GrDirectContext::SetMemoryOverCheckProc(MemoryOverCheckProc proc) {
    fMemoryOverCheckProc = proc;
}

void GrDirectContext::resetContext(uint32_t state) {
    ASSERT_SINGLE_OWNER
    fGpu->resetContext(state);
}
...
void GrDirectContext::freeGpuResources() {
    ...
    this->getTextBlobRedrawCoordinator()->freeAll();
...
void GrDirectContext::popGrResourceTag()
{
    if (fResourceCache) {
        return fResourceCache->popGrResourceTag();
    }
}
