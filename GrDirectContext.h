...
public:
    ...
    using MemoryOverCheckProc = std::function<void(GrDirectContext*)>;
    void SetMemoryOverCheckProc(MemoryOverCheckProc proc);
    MemoryOverCheckProc GetMemoryOverCheckProc() const { return fMemoryOverCheckProc; }
...
private:
    ...
    // true if the PM/UPM conversion succeeded; false otherwise
        ...
        // This guard is passed to the GrDrawingManager and, from there, to all the GrOpsTasks
        mutable GrSingleOwner fSingleOwner;

        MemoryOverCheckProc fMemoryOverCheckProc = nullptr;

        const GrContextOptions::PersistentCache* fPersistentCache = nullptr;

        bool fSyncCpuPending = false;

        std::unique_ptr<GrClientMappedBufferManager> fMappedBufferManager;
    std::unique_ptr<GrAtlasManager> fAtlasManager;
    std::function<void(const std::vector<int>&, const std::string&, bool)> vulkanErrorCallback_;
