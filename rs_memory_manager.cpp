...
void MemoryManager::MemoryOverCheck(Drawing::GPUContext* gpuContext)
{
    ...
    auto task = [gpuMemory = std::move(gpuMemory)]() {
        ...
        for (const auto& [pid, info] : infoMap) {
            totalMemory += info.TotalMemory();
        }
        constexpr uint64_t MEMORY_DUMP_THRESHOLD = 3ULL * 1024 * 1024 * 1024;
        static uint64_t lastDumpTime = 0;
        if (totalMemory >= MEMORY_DUMP_THRESHOLD && currentTime > lastDumpTime) {
            lastDumpTime = currentTime + MEMORY_REPORT_INTERVAL;
            RSMainThread::Instance()->PostTask([totalMemory]() {
                RS_TRACE_NAME_FMT("RSMem Dump Task Total > 3GB");
                std::unordered_set<std::u16string> argSets;
                std::string dumpString = "";
                std::string type = MEM_GPU_TYPE;
                RSMainThread::Instance()->DumpMem(argSets, dumpString, type, 0);

                std::string filePath = "/data/service/el0/render_service/renderservice_mem_3gb.txt";
                std::string gpuMemInfo = "";
                std::ifstream gpuMemInfoFile(GPUMEM_INFO_PATH);
                if (gpuMemInfoFile.is_open()) {
                    std::stringstream buffer;
                    buffer << gpuMemInfoFile.rdbuf();
                    gpuMemInfo = buffer.str();
                    gpuMemInfoFile.close();
                }

                std::ofstream tempFile(filePath);
                if (tempFile.is_open()) {
                    tempFile << "RS Total Memory Over 3GB: " << totalMemory << "\n";
                    tempFile << "\n******************************\n";
                    tempFile << gpuMemInfo;
                    tempFile << "\n************ endl ************\n";
                    tempFile << "\n******************************\n";
                    ...
                    tempFile << dumpString;
                    tempFile.close();
                    RS_LOGE("RS Total Memory Over 3GB. Dumped to %{public}s", filePath.c_str());
                } else {
                    RS_LOGE("Failed to open file %{public}s", filePath.c_str());
                }
            });
        }
