...
void SkiaGPUContext::SetMemoryOverCheckProc(std::function<void(void*)> proc)
{
    if (grContext_ != nullptr) {
        grContext_->SetMemoryOverCheckProc([proc](GrDirectContext* context) {
            proc(static_cast<void*>(context));
        });
    }
}
